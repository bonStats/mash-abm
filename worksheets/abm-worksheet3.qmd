...
Our linear model is (with $p=20$, $n=60$)
$$\begin{aligned}
...
$$m(y|X_\gamma) = (g+1)^{-(p_\gamma+1)/2}\pi^{-n/2}\Gamma(n/2) \times\left[y^\top y-\frac{g_0}{g_0+1}y^\top X_\gamma(X_\gamma^\top X_\gamma)^{-1}X_\gamma^\top y\right]^{-n/2}$$
where $p_\gamma = \sum_{i=1}^p\gamma_i$, the number of non-zero coefficients (excluding the intercept). 

1. For $g=0.1, 1, 10, 100, 1000$, give $E[\sigma^2|y, X]$ and $E[\beta_0|y, X]$. What can you conclude about the impact of the prior on the posterior?
...
1. Use the \texttt{bms} function from package [{BMS}](https://cran.r-project.org/package=BMS) to sample from the posterior distribution of $\gamma$, and conclude on the most likely model.
