...
- Syntax to code joint probability model

```{mermaid}
%%| fig-align: center
flowchart LR
  A(PPL model specification) --> B(log probability)
...
$$\log p(\theta, y) = \log \pi(\theta) + \log p(y \mid \theta)$$

$$\pi(\theta \mid y) \propto p(\theta, y)$$
